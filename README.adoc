...
** TLS security is Enabled (_please don't use in production!_)
** compression is Enabled for gRPC server (enabled by default)
- JSON RPC access through 0.0.0.0:8545 (both HTTP and WebsScket)
** proxy requests to Ethereum and Kovan upstreams
** request path for Ethereum Mainnet is `/eth`, `/mainnet` for Mainnet Mainnet, and `/btc` for bitcoin
