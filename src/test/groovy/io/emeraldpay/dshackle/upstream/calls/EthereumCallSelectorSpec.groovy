...
class EthereumCallSelectorSpec extends Specification {
    ...
    def "Get height matcher for latest balance"() {
        ...
        when:
        def act = callSelector.getMatcher("eth_getBalance", '["0x0000", "latest"]'.getBytes(), head, false).block()
        then:
    ...
    def "Get height matcher for latest call"() {
        ...
        when:
        def act = callSelector.getMatcher("eth_call", '[{"from":"0x0000"}, "latest"]'.getBytes(), head, false).block()
        then:
        act == new Selector.HeightMatcher(100)
    }
    ...
    def "Get height matcher for latest storageAt"() {
        ...
        when:
        def act = callSelector.getMatcher("eth_getStorageAt", '["0x295a70b2de5e3953354a6a8344e616ed314d7251", "0x0", "latest"]'.getBytes(), head, false).block()
        then:
        act == new Selector.HeightMatcher(100)
    }
    ...
    def "Get height matcher for balance on block"() {
        ...
        when:
        def act = callSelector.getMatcher("eth_getBalance", '["0x0000", "0x40"]'.getBytes(), head, false).block()
        then:
    ...
    def "Get height matcher for balance on block referred by hash"() {
        ...
        when:
        def act = callSelector.getMatcher("eth_getBalance", '["0x0000", "0xc90f1c8c125a4d5b90742f16947bdb1d10516f173fd7fc51223d10499de2a812"]'.getBytes(), head, false).block()
        then:
    ...
    def "No matcher for invalid height"() {
        ...
        def act = callSelector.getMatcher("eth_getBalance", '["0x0000", "0x10000000000000000"]'.getBytes(), head, false).block()
        then:
        act == null
    }
    ...
    def "No matcher for negative height"() {
        ...
        when:
        def act = callSelector.getMatcher("eth_getBalance", '["0x0000", "-0x100"]'.getBytes(), head, false).block()
        then:
        act == null
    }
    ...
    def "No matcher for negative long"() {
        ...
        def act = callSelector.getMatcher("eth_getBalance", '["0x0000", "0x8000000000000000"]'.getBytes(), head, false).block()
        then:
        act == null
    }
    ...
    def "No matcher for pending balance"() {
        ...
        when:
        def act = callSelector.getMatcher("eth_getBalance", '["0x0000", "pending"]'.getBytes(), head, false).block()
        then:
        act == null
    }
    ...
    def "Get height matcher with EIP-1898"() {
        ...
        def head = Stub(Head)
        when:
        def act = callSelector.getMatcher("eth_call", '["0x0000", {"blockNumber": "0x100"}]'.getBytes(), head, false).block()
        then:
    ...
    def "Get hash matcher with EIP-1898"() {
        ...
        def act = callSelector.getMatcher("eth_call",
                '["0x0000", {"blockHash": "0xa6af163aab691919c595e2a466f0a7b01f1dff8cfd9631dee811df57064c2d32"}]'.getBytes(), head, false)
                .block()
        then:
    ...
    def "Get empty matcher for block tag with passthrough arg"() {
        ...
        when:
        def act = callSelector.getMatcher("eth_call",
                '["0x0000", {"blockHash": "0xa6af163aab691919c595e2a466f0a7b01f1dff8cfd9631dee811df57064c2d32"}]'.getBytes(), head, true)
                .block()
        then:
        act == null
    }
    ...
    def "Get same matcher for getFilterChanges method"() {
        ...
        expect:
        callSelector.getMatcher("eth_getFilterChanges", param.getBytes(), head, true).block()
    ...
    def "Get empty matcher for getFilterChanges method without params"() {
        ...
        when:
        def act = callSelector.getMatcher("eth_getFilterChanges", "[]".getBytes(), head, false).block()
    ...
    def "Get height matcher for getByHash and getTransactionByBlockHash methods"() {
        ...
        def act = callSelector.getMatcher(
                method, '["0xa6af163aab691919c595e2a466f0a7b01f1dff8cfd9631dee811df57064c2d32", false]'.getBytes(),
                head, false
        )
    ...
    def "No height matcher for getByNumber and getTransactionByBlockNumber methods when passthrough is on"() {
        ...
        when:
        def act = callSelector.getMatcher(
                method, params.getBytes(), head, true
        )
    ...
    def "Get height matcher for byNumber methods"() {
        ...
        when:
        def act = callSelector.getMatcher(
                method, params.getBytes(), head, false
        )
    ...
    def "No height matcher for getByHash method"() {
        ...
        when:
        def act = callSelector.getMatcher(
                "eth_getBlockByHash", '["0xa6af163aab691919c595e2a466f0a7b01f1dff8cfd9631dee811df57064c2d32", false]'.getBytes(),
                head, false
        )
    ...
    def "Get height matcher for getLogs and eth_newFilter method"() {
        ...
        when:
        def act = callSelector.getMatcher(method, param.getBytes(), head, false)
    ...
    def "No height matcher for getLogs and eth_newFilter method when passthrough is on"() {
        ...
        when:
        def act = callSelector.getMatcher(method, param.getBytes(), head, true)
