...
object EthereumChainSpecific : AbstractPollChainSpecific() {
    ...
    override fun parseBlock(
        ...
        upstreamId: String,
        api: ChainReader,
    ): Mono<BlockContainer> = Mono.just(BlockContainer.fromEthereumJson(data, upstreamId))

    override fun getFromHeader(
        data: ByteArray,
        upstreamId: String,
        api: ChainReader,
    ): Mono<BlockContainer> = parseBlock(data, upstreamId, api)

    override fun latestBlockRequest() = ChainRequest("eth_getBlockByNumber", ListParams("latest", false))

    override fun listenNewHeadsRequest(): ChainRequest = ChainRequest("eth_subscribe", ListParams("newHeads"))

    override fun unsubscribeNewHeadsRequest(subId: String): ChainRequest = ChainRequest("eth_unsubscribe", ListParams(subId))
    ...
    ): Mono<ChainReader> = Mono.just(EthereumLocalReader(cachingReader as EthereumCachingReader, methods, head, logsOracle))
    ...
    override fun makeCachingReaderBuilder(tracer: Tracer): CachingReaderBuilder =
        { ms, caches, methodsFactory -> EthereumCachingReader(ms, caches, methodsFactory, tracer) }
    ...
    override fun lowerBoundService(
        chain: Chain,
        upstream: Upstream,
    ): LowerBoundService = EthereumLowerBoundService(chain, upstream)

    override fun finalizationDetectorBuilder(): FinalizationDetector = EthereumFinalizationDetector()

    override fun upstreamSettingsDetector(
        chain: Chain,
        upstream: Upstream,
    ): UpstreamSettingsDetector = EthereumUpstreamSettingsDetector(upstream, chain)

    override fun makeIngressSubscription(ws: WsSubscriptions): IngressSubscription = EthereumWsIngressSubscription(ws)

    override fun callSelector(caches: Caches): CallSelector = EthereumCallSelector(caches)
}
