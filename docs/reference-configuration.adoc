...
auth:
  enabled: true
  public-key-owner: "token-issuer-name"
  server:
...
| `public-key-owner`
|
| Human-readable identifier of the JWT-issuing party.

| `server.provider-private-key`
|
| Path to a PKCS#8 RSA private key used for signing gRPC responses (see <<response-signing>>).

| `server.external-public-key`
|
| Path to the PEM-encoded RSA public key of the token issuer used to verify inbound JWT tokens.
|===
